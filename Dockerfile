...
RUN R -e "install.packages(c('BiocManager','https://cran.r-project.org/src/contrib/Archive/hyperSpec/hyperSpec_0.99-20180627.tar.gz'), repos='https://cloud.r-project.org', dependencies=T); BiocManager::install('GenomeInfoDbData'); BiocManager::install('AneuFinder')"
...
COPY scgs_py27.txt /
RUN conda create --name scgs_py27 --file /scgs_py27.txt nomkl && conda clean -y -a && rm -rf /opt/conda/envs/scgs_py36/pkgs/*
...
# Install Funannotate
#RUN [ "/bin/bash", "-c", "source activate scgs_py27 && conda install -c bioconda -c conda-forge trnascan-se=1.3.1 funannotate=1.7.2 && conda clean -y -a && rm -rf /opt/conda/pkgs/* && source deactivate" ]
