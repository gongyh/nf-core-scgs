...
process REBLOBTOOLS {
    ...
    input:
    tuple(meta: Map, contigs: Path, anno: Path, uniprot_anno: Path, has_uniprot: Boolean, bam: Path, bai: Path)
    db: Path

    output:
    record(meta: meta, tax: file("*/*.blobDB*table.txt"), contigs: file("*.fasta", includeInputs: true), tax_split: file("*", type: "dir"), versions: file("versions.yml"))
    topic:
