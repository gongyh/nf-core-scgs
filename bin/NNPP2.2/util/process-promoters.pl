...
sub ProcessPromoters {
     ...
     for ($i = 0; $i <= $#hits; $i++) { 
	  ...
	  # Look for hit
	  elsif (/^Hit ([0-9]+): position.* ([0-9]+) \.\. ([0-9]+), (.*) = ([0-9]+)/i) {
	       ++$h;
	       $promoter[$h]{start} = $2;
	       $promoter[$h]{end} = $3;
	       $promoter[$h]{signal} = $5;
     ...
     if ($highest_only) {
	  return(&RemoveOverlappingHits(*promoter, $seq_length, $highest_only));
     }
     else {
	  return(@promoter);
     }
...
sub RemoveOverlappingHits {
     ...
     if ($DEBUG) {
	  ...
	  for ($p = 0; $p <= $#forward; $p++) {
	       print("$forward[$p]{start}\t$forward[$p]{end}\t$forward[$p]{score}\n");
	  }

	  print("Reverse strand:\nStart\tEnd\tScore\n");
	  for ($p = 0; $p <= $#reverse; $p++) {
	       print("$reverse[$p]{start}\t$reverse[$p]{end}\t$reverse[$p]{score}\n");
	  }
     }
     ...
     if ($#reverse >= 0) {
	  ...
	  while ($#sorted_by_score >= 0) {
	       # pick_best_promoters destructively modifies both arrays!
	       &pick_best_promoters(*sorted_by_score, *best_reverse, $neighborhood);
	  }
	  # Resort promoters by start position
...
sub pick_best_promoters {
     ...
     for ($p = 0; ($p <= $#prom) && ($prom[$p]{score} == $prom[0]{score}); $p++) {
	  ...
	  for ($j = $p; $j <= $#prom; $j++) {
	       if (&in_neighborhood($prom[$j]{start}, $keep[-1]{start}, $neighborhood)) {
		    if ($DEBUG) { print("Promoter $j ($prom[$j]{start}) is in neighborhood of current best ($keep[-1]{start})\n");
				  print("Remaining $#prom+1 promoters after removing $prom[$j]{start}: "); }
		    splice(@prom, $j, 1);
		    if ($DEBUG) {
			 for ($k = 0; $k <= $#prom; $k++) {
			      print("$prom[$k]{start} ");
