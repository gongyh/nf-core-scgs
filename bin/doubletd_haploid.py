...
class doubletFinder():
    ...
    def __init__(self, df_total, df_alt, cells, delta, beta, mu_hom, alpha_fp, alpha_fn, missing=False, verbose=True, binom=False, precision=None):
        ...
        if (self.df_total.values - self.df_alt.values).min() < 0:
            raise Exception(
                'total reads must be greater than or equal to alternate reads!')

        self.Sigma = ((0, 1), (0, 1/2, 1))
        self.Theta = ((0, 1), (0, 1/2, 1))

        self.px_z = {x: 0 for z in [0, 1] for x in itertools.product(
            self.muts, self.Sigma[z], [z])}
        ...
        if self.alpha_fn == None or self.alpha_fp == None or self.precision == None:
            ...
            if self.precision == None:
                mean3, prec3 = getBetaMOM(
                    vaf_values[(vaf_values > 0.15) & (vaf_values < 0.85)])
       ...
       #         self.px_z[m, c, 1] /= norm_const[m]
        ...
        self.py_xz = {x: 0 for z in [0, 1] for x in itertools.product(
            self.Theta[z], self.Sigma[z], [z])}
    ...
    def likelihood(self):
        ...
        for cell in self.cells:
            if self.doublet_result[cell] == 'doublet':
                likelihood += self.logprobs[cell, 1] - \
                    self.logprobs[cell, 0] - self.threshold
        return likelihood
...
def main(args):
    ...
    valid_cols = np.logical_and(
        np.any(vaf_values > 0.85, axis=0), np.any(vaf_values < 0.15, axis=0))
    ...
    solver = doubletFinder(df_total, df_alt, cells, delta=args.delta, beta=args.beta, missing=args.missing,
                           mu_hom=args.mu_hom, alpha_fp=args.alpha_fp, alpha_fn=args.alpha_fn,
...
def get_options():
    ...
    parser.add_argument("--noverbose", dest="verbose",
                        help="do not output statements from internal solvers [default is false]", action='store_false')
    parser.add_argument("--binomial", dest="binom",
                        help="use binomial distribution for read count model [default is false]", action='store_true')
    ...
    parser.add_argument("--missing", dest="missing",
                        help="use missing data in the model? [No]", action='store_true')
