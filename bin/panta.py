    ...
    if args.tsv != None:
        with open(args.tsv, "r") as fh:
            ...
            for row in csv_reader:
                ...
                samples.append({"id": str(sample_id), "name": sample_name, "gff_file": gff, "assembly": assembly})
                sample_id += 1
    elif args.gff != None:
        gff_list = args.gff
        for gff in gff_list:
            ...
            if sample_name in previous_samples:
                ...
                previous_samples.append(sample_name)
            check_dir_exist(gff)
            samples.append({"id": str(sample_id), "name": sample_name, "gff_file": gff, "assembly": None})
            sample_id += 1
        ...
        for fasta in fasta_list:
            ...
            samples.append({"id": str(sample_id), "name": sample_name, "gff_file": None, "assembly": fasta})
            sample_id += 1
    else:
...
def init_function(args):
    ...
    # call Init pipeline
    wrapper.run_init_pipeline(samples, collection_dir, temp_dir, baseDir, args, timing_log, resume)
...
def main():
    ...
    parser.add_argument(
        "-g", "--gff", help="genome annotation input files (e.g. from Prokka)", default=None, nargs="*", type=str
    )
    ...
    parser.add_argument(
        "-o", "--outdir", help="output directory or directory of previous collection ", required=True, type=str
    )
    parser.add_argument(
        "-d",
        "--diamond",
    ...
    parser.add_argument("-i", "--identity", help="minimum percentage identity (0..100)", default=95, type=float)
    parser.add_argument("-c", "--coverage", help="minimum percentage coverage (0..100)", default=0, type=float)
    parser.add_argument("-e", "--evalue", help="Blast evalue", default=1e-6, type=float)
    parser.add_argument("-t", "--threads", help="number of threads to use, 0 for all", default=0, type=int)
    parser.add_argument("--table", help="codon table", default=11, type=int)
    parser.add_argument("-s", "--split", help="Split paralogs from ortholog", default=False, action="store_true")
    parser.add_argument(
        "-r", "--resume", help="Resume the analysis from interuption", default=False, action="store_true"
    )
    parser.add_argument(
        "-as", "--addstrand2gene", help="Add strand direction (+, -) to gene name", default=False, action="store_true"
    )
    # Execute parse_args()
