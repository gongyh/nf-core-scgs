...
def create_parser():
    ...
    parser.add_argument(
        "-i", "--fna_file", required=False, help="fasta genome sequence (if not specified, faa_file must be specified)."
    )
    parser.add_argument(
        "-a",
        "--faa_file",
        required=False,
        help="fasta amino acids sequence predicted by Prodigal (if not specified, fna_file must be specified).",
    )
    parser.add_argument(
        "-t",
        "--path",
        required=False,
        help="optional folder path where output files go (if not specified, the input file path is used)",
    )
    parser.add_argument(
        "-n",
        "--n_sample",
        required=False,
        help="optional, the number of samples to generate a random combination of convergent and divergent distances, default 10**4)",
    )
...
def read_faa(faa):
    ...
    df["nc"] = df["nc"].replace(
        r">|_\d+\s+", "", regex=True
    )  ## extract the contig name. This works for complete genomes and metagenomes
