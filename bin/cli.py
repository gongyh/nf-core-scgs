...
def tools_split(results_dir: Path = typer.Option(
    ...
    if prokka_dir.exists() and prokka_dir.is_dir():  # exist the dir, ok
        ...
        typer.echo(
            f"INFO: gene annotations not found, will skip splitting gene ids.")
        anno_exist = False
    ...
    if kofam_dir.exists() and kofam_dir.is_dir():
        ...
        typer.echo(
            f"INFO: KO annotations not found, will skip splitting gene kos.")
        ko_exist = False
...
def tools_checkm(fastas_dir: Path = typer.Option(
    ...
    if checkm_table.exists() and checkm_table.is_file():
        if force:
            ...
            typer.secho(f"Output checkm results already exist.",
                        fg=typer.colors.RED)
            raise typer.Abort()
...
def tools_fastANI(query_genome: Path = typer.Option(
    ...
    subprocess.check_call(" ".join(
        ["fastANI", params_str, "--output", str(output_file), "--threads", str(threads)]), shell=True)
    if visualize:
...
def tools_roary(input_dir: Path = typer.Option(
    ...
    if len(gffs) < 3:
        typer.secho(f"Error: at least 3 gff files needed.",
                    fg=typer.colors.RED)
        raise typer.Abort()
    ...
    subprocess.check_call(" ".join(["roary", params_str, "-f", str(
        output_dir), "-p", str(threads), str(input_dir)+"/*.gff"]), shell=True)
    gpa = output_dir.joinpath("gene_presence_absence.csv")
    if not gpa.exists():
        typer.secho(f"Error: gene_presence_absence.csv not generated",
                    fg=typer.colors.RED)
        raise typer.Abort()
    aln = output_dir.joinpath("core_gene_alignment.aln")
    if not aln.exists():
        typer.secho(f"Error: no core gene alignment file generated.",
                    fg=typer.colors.RED)
        raise typer.Abort()

    typer.echo(f"Generate a newick tree.")
    subprocess.check_call(" ".join(
        ["fasttree", "-nt", "-gtr", str(aln), ">", str(output_dir)+"/tree.newick"]), shell=True)
