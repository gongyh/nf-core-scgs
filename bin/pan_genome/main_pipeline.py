...
def run_cd_hit(faa_file, out_dir, threads, timing_log, resume):
    ...
    cmd = (
        f"cd-hit -i {faa_file} -o {cd_hit_represent_fasta} -s 0.98 -c 0.98 "
        f"-T {threads} -M 0 -g 1 -d 256 > /dev/null"
    )

    statement = os.path.isfile(cd_hit_represent_fasta) and os.path.isfile(cd_hit_cluster_file) and resume[0] == True
    if not statement:
...
def run_blast(
    database_fasta, query_fasta, out_dir, timing_log, evalue, max_target_seqs, identity, query_coverage, threads, resume
):
    """
    ...
    cmd = f"makeblastdb -in {database_fasta} -dbtype " f"prot -out {blast_db} -logfile /dev/null"
    utils.run_command(cmd, timing_log)

    # run blast
    blast_result_temp = os.path.join(out_dir, "blast_results.tmp")
    cmd = (
        f"blastp -query {query_fasta} -db {blast_db} -evalue {evalue} "
...
def run_diamond(
    ...
    cmd = f"diamond makedb --in {database_fasta} -d {diamond_db} " f"-p {threads} --quiet"
    utils.run_command(cmd, timing_log)

    # run diamond blastp
    diamond_result_tmp = os.path.join(out_dir, "diamond_results.tmp")
    cmd = (
        f"diamond blastp -q {query_fasta} -d {diamond_db} -p {threads} "
...
def pairwise_alignment(
    ...
    logging.info(
        f"Comparing {query_result.stdout.rstrip()} sequences " f"with {database_result.stdout.rstrip()} sequences"
    )
...
def cluster_with_mcl(blast_result, out_dir, timing_log, resume):
    ...
    cmd = (
        f"mcxdeblast --m9 --score r --line-mode=abc {blast_result} "
