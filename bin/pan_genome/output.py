...
def write_summary(count, out_dir, total_samples):
    ...
    with open(summary_file, "w") as fh:
        fh.write("Core genes" + "\t" + "(99% <= strains <= 100%)" + "\t" + str(count[0]) + "\n")
        fh.write("Soft core genes" + "\t" + "(95% <= strains < 99%)" + "\t" + str(count[1]) + "\n")
        fh.write("Shell genes" + "\t" + "(15% <= strains < 95%)" + "\t" + str(count[2]) + "\n")
        fh.write("Cloud genes" + "\t" + "(0% <= strains < 15%)" + "\t" + str(count[3]) + "\n")
...
def output_cluster_info(clusters, clusters_annotation, gene_dictionary, samples, out_dir):
    """
...
def update_cluster_info(
    previous_clusters, new_clusters, new_clusters_annotation, gene_dictionary, new_samples, temp_dir, collection_dir
):
    """
    ...
    with open(old_cluster_info_file, "r") as in_fh_1, open(new_cluster_info_file, "w") as out_fh_1:
...
def update_rtab(old_file, previous_clusters, new_clusters, gene_dictionary, new_samples, temp_dir):
    starttime = datetime.now()
