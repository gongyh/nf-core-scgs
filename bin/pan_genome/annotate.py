...
def search_sequence(faa_file, threads, database, out_dir, timing_log):
    ...
    if database["tool"] == "blastp":
        ...
        cmd = (
            f"blastp -query {faa_file} -db {db_dir} -num_threads {threads} "
        ...
        evalue = database["EVALUE"]
        cmd = (
            f"cat {faa_file} | parallel --gnu --plain -j {threads} "
    ...
    for qresult in SearchIO.parse(result_file, fmt):
        ...
        for hit in qresult:
            ...
            if desc[2] != "":
                ...
                product = None
            search_result[qseqid] = {"id": sseqid, "gene": gene_name, "product": product}
...
def annotate_cluster_fasta(unlabeled_clusters, rep_fasta, temp_dir, baseDir, timing_log, threads):
    ...
    for database in ordered_database:
        # search
        out_file = search_sequence(faa_file, threads, database, out_dir, timing_log)
        # parse result
