...
def run_cd_hit_2d(database_1, database_2, out_dir, threads, timing_log, resume):
    ...
    cmd = (
        f"cd-hit-2d -i {database_1} -i2 {database_2} -o {notmatch_faa} "
        f"-s 0.98 -s2 0.98 -c 0.98 -T {threads} -M 0 -g 1 -d 256 > /dev/null"
    )

    statement = os.path.isfile(notmatch_faa) and os.path.isfile(cd_hit_cluster_file) and resume[0] == True
    if not statement:
...
def add_gene_blast(previous_clusters, cd_hit_clusters, blast_result, fasta_file, out_dir):
    """
    ...
    utils.create_fasta_exclude(fasta_file_list=[fasta_file], exclude_list=match_dict.keys(), output_file=remain_fasta)
